...
const capturePointSchema = new mongoose.Schema({
    ...
    sector: Number,
    capturedBy: String,
    protectionTimer: Date
});

const CapturePoint = mongoose.model('CapturePoint', capturePointSchema);

// Connexion MongoDB (Railway fournit MONGO_URL automatiquement)
const MONGODB_URI = process.env.MONGO_URL || process.env.MONGODB_URI || 'mongodb://localhost:27017/capture-drapeau';
...
app.get('/api/init', async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            message: '✅ Base de données initialisée avec succès sur Railway !',
